...
import { fetchAllProperties, fetchPropertyById, setPropertyStatus } from "../apis/properties";
...
const useFetchProperties = () =>
  useQuery([QUERY_KEYS.PROPERTIES_LIST], fetchAllProperties, {
    select: data => data?.docs.map(doc => ({ ...doc.data(), id: doc.id, key: doc.id})),
...
const mutatePropertyStatus = (propertyId, status) => useMutation(()=>setPropertyStatus(propertyId, status))
